...
export function AdminProductsPanel({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  useEffect(() => {
    fetch('/api/products/admin').then(async (response) => ({ response, data: await response.json() })).then(({ response, data }) => {
  ...
  return <section className="admin-products-grid" aria-label={t('admin.catalog')}>
    <form className="admin-panel admin-product-form" onSubmit={submit}><div className="admin-section-heading"><div><span>{t('admin.catalog')}</span><h2>{editingSlug ? t('admin.edit') : t('admin.add')}</h2></div>{editingSlug ? <button type="button" className="admin-icon-button" onClick={resetForm} aria-label={t('admin.cancel')}><X size={21} /></button> : <Plus size={22} />}</div>
      <label>{t('admin.nameId')}<input required minLength={3} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label>
      <label>{t('admin.nameEn')} <span>({t('admin.optional')})</span><input minLength={3} value={form.nameEn} onChange={(event) => setForm({ ...form, nameEn: event.target.value })} /></label>
      ...
      <div className="admin-form-row"><label>{t('admin.price')}<input required type="number" min="0" value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} /></label><label>{t('admin.stock')}<input required type="number" min="0" value={form.stock} onChange={(event) => setForm({ ...form, stock: event.target.value })} /></label></div>
      ...
      <label>{t('admin.image')} <span>({t('admin.optional')})</span><input type="url" value={form.imageUrl} onChange={(event) => setForm({ ...form, imageUrl: event.target.value })} placeholder="https://" /></label>
    ...
    <div className="admin-panel admin-products-list"><div className="admin-section-heading"><div><span>{t('admin.saved')}</span><h2>{t('admin.list')}</h2></div><select aria-label={t('admin.category')} value={category} onChange={(event) => setCategory(event.target.value as 'ALL' | ProductCategory)}><option value="ALL">{t('admin.allCategories')}</option>{PRODUCT_CATEGORIES.map((item) => <option key={item.id} value={item.id}>{language === 'en' ? item.labelEn : item.label}</option>)}</select></div>
