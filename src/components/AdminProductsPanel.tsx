...
export function AdminProductsPanel({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/products/admin')
      ...
      .catch((error) => {
        if (active) setMessage(error instanceof Error ? error.message : t('admin.loadError'));
      });
    return () => { active = false; };
  }, [onUnauthorized, t]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const data = await readJsonResponse(response, t('admin.loadError'));
      if (!response.ok) throw new Error(data.message || t('admin.loadError'));
      setProducts((current) => editingSlug
        ? current.map((product) => product.slug === editingSlug ? data.product : product)
  ...
  return (
    <section className="admin-products-grid" aria-label={t('admin.catalog')}>
      <form className="admin-panel admin-product-form" onSubmit={submit}>
        <div className="admin-section-heading">
          <div><span>{t('admin.catalog')}</span><h2>{editingSlug ? t('admin.edit') : t('admin.add')}</h2></div>
          {editingSlug ? (
            <button type="button" className="admin-icon-button" onClick={resetForm} aria-label={t('admin.cancel')}><X size={21} /></button>
        ...
        <label>{t('admin.category')}
          <select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value as ProductCategory })}>
            {PRODUCT_CATEGORIES.map((item) => <option key={item.id} value={item.id}>{language === 'en' ? item.labelEn : item.label}</option>)}
          </select>
        </label>
        ...
        <div className="admin-form-actions">
          <button className="admin-primary-button" disabled={saving}>{editingSlug ? <Pencil size={18} /> : <Plus size={18} />} {saving ? t('admin.saving') : editingSlug ? t('admin.save') : t('admin.create')}</button>
          {editingSlug && <button type="button" className="admin-secondary-button" onClick={resetForm}>{t('admin.cancel')}</button>}
        </div>
      </form>

      <div className="admin-panel admin-products-list">
        <div className="admin-section-heading">
          <div><span>{t('admin.saved')}</span><h2>{t('admin.list')}</h2></div>
          <select aria-label={t('admin.category')} value={category} onChange={(event) => setCategory(event.target.value as 'ALL' | ProductCategory)}>
        ...
        <div className="admin-product-cards">
          {shownProducts.length ? shownProducts.map((product) => (
            <article key={product.slug} className={product.active === false ? 'inactive' : ''}>
              <div className="admin-product-copy">
                <strong>{displayName(product)}</strong>
                <span>{categoryLabel(product.category, language)}</span>
                <span className={`admin-product-status ${product.active === false ? 'inactive' : ''}`}>{product.active === false ? t('admin.inactive') : t('admin.active')}</span>
              </div>
              <div className="admin-product-meta"><strong>Rp {product.price.toLocaleString('id-ID')}</strong><span>{t('admin.stock')} {product.stock}</span></div>
              <div className="admin-product-actions">
                <button type="button" onClick={() => startEdit(product)}><Pencil size={16} /> {t('admin.editAction')}</button>
                <button type="button" className={product.active === false ? 'restore' : 'danger'} disabled={actionSlug === product.slug} onClick={() => void toggleActive(product)}>
                  {product.active === false ? <RotateCcw size={16} /> : <Trash2 size={16} />}{product.active === false ? t('admin.restore') : t('admin.disable')}
                </button>
              </div>
            </article>
          )) : (
            <div className="admin-empty-products"><Boxes size={34} /><p>{t('admin.empty')}</p></div>
          )}
        </div>
      </div>
    </section>
