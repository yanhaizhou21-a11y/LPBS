...
export function Navbar({ cartQty, onOpenCart, onOpenCheckout }: NavbarProps) {
  ...
  const links = [
    { label: t('nav.home'), href: '/', icon: Home }, { label: t('nav.products'), href: '/products', icon: Package },
  ...
  return <>
    <motion.header className="notch-navbar" aria-label="Main navigation" animate={{ y: visible ? 0 : -82 }} transition={reduceMotion ? { duration: 0 } : { type: 'spring', stiffness: 420, damping: 38 }}>
      <div className="notch-side" aria-hidden="true" /><div className="notch-shell"><NotchCurve side="left" /><div className="notch-content">
        ...
        <nav className="notch-links" aria-label="Primary navigation">{links.slice(0, 3).map(({ label, href, icon: Icon }) => <a key={href} href={href}><Icon size={16} /><span>{label}</span></a>)}</nav>
        <a href="/" className="notch-brand" aria-label="Botani Seed"><img src={ASSETS.logo} alt="" /><span>Botani Seed</span></a>
        <nav className="notch-links notch-links-right" aria-label="Secondary navigation">{links.slice(3).map(({ label, href, icon: Icon }) => <a key={href} href={href}><Icon size={16} /><span>{label}</span></a>)}<LanguageToggle className="notch-language-toggle" /><ThemeToggleButton className="notch-theme-toggle" /><button className="notch-cart" onClick={onOpenCart} aria-label={`${t('nav.cart')}: ${cartQty}`}><ShoppingBag size={18} /><span className="notch-cart-label">{t('nav.cart')}</span>{cartQty > 0 && <span className="notch-cart-badge">{cartQty}</span>}</button><button className="notch-order" onClick={onOpenCheckout}>{t('nav.order')}</button></nav>
