...
import { ArrowLeft, Leaf, MessageCircle, PackageSearch, Search } from 'lucide-react';
import { FEATURED_PRODUCTS, PRODUCT_CATEGORIES, Product, ProductCategory, categoryLabel } from '../data/products';

export function ProductsPage({ onGoHome }: { onGoHome: () => void }) {
  ...
  return <main className="products-page">
    ...
    <section className="products-catalog container" aria-labelledby="catalog-title">
      ...
      {filtered.length ? <div className="product-grid">{filtered.map((p) => <article className="catalog-product-card" key={p.slug}><div className="catalog-product-visual">{p.imageUrl ? <img src={p.imageUrl} alt={p.name} loading="lazy" /> : <Leaf size={46} aria-hidden="true" />}<span>{categoryLabel(p.category)}</span></div><div className="catalog-product-body"><h2>{p.name}</h2><p>{p.description}</p><div className="catalog-product-meta"><strong>Rp {p.price.toLocaleString('id-ID')}</strong><span>{p.stock > 0 ? `Stok ${p.stock}` : 'Hubungi admin'}</span></div><a href={`https://wa.me/6281299450708?text=${encodeURIComponent(`Halo Botani Seed, saya tertarik dengan ${p.name}.`)}`} target="_blank" rel="noreferrer"><MessageCircle size={18} /> Tanya produk</a></div></article>)}</div> : <div className="products-empty"><PackageSearch size={38} /><h2>Produk belum ditemukan</h2><p>Coba kategori atau kata pencarian lainnya.</p></div>}
    </section>
  </main>;
}
